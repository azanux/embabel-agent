...
|`SPRING_OBSERVATION`
|Tracing backend (`SPRING_OBSERVATION`, `MICROMETER_TRACING`, or `OPENTELEMETRY_DIRECT`)
...
NOTE: `SPRING_OBSERVATION` is the recommended implementation as it provides both traces and metrics.
The other modes provide traces only.
